...
@pytest.mark.asyncio
async def test_fetch_with_vectors_with_metadata_async(async_index: AsyncIndex):
    v1_id = "v1-id1"
    v1_metadata = {"metadata_field": "metadata_value"}
    v1_values = [0.1, 0.2]
...
@pytest.mark.asyncio
async def test_fetch_with_vectors_without_metadata_async(async_index: AsyncIndex):
    v1_id = "v2-id1"
    v1_metadata = {"metadata_field": "metadata_value"}
    v1_values = [0.1, 0.2]
    ...
    res = await async_index.fetch(
        ids=[v1_id, v2_id], include_vectors=True, include_metadata=False
    )
...
@pytest.mark.asyncio
async def test_fetch_without_vectors_without_metadata_async(async_index: AsyncIndex):
    v1_id = "v3-id1"
    v1_metadata = {"metadata_field": "metadata_value"}
    v1_values = [0.1, 0.2]
    ...
    res = await async_index.fetch(
        ids=[v1_id, v2_id], include_vectors=False, include_metadata=False
    )
